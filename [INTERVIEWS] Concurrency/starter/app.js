...
// Put your code here
// ------------------
async function displayFollers() {
  const followerIds = await fetchFollowerIds();
  const userData = await Promise.all(
    followerIds.map(async (id) => await fetchUserData(id))
  );
  const followersWrapper = document.querySelector(".followers");
  const followerHtml = userData.map(
  ...
  followersWrapper.innerHTML = followerHtml.join("");
}
displayFollers();
// ------------------
